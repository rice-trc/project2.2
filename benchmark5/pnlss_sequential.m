...
load(sprintf('data/b%d_A%d_up%d_%s',benchmark,Alevels(1),upsamp,dataname))
...
for ia=1:length(Alevels)
    load(sprintf('data/b%d_A%d_up%d_%s',benchmark,Alevels(ia),upsamp,dataname), 'u', 'y')
...
%  	[~, y_mod, models_pnlss] = fLMnlssWeighted(uc, yc, models_pnlss(end-5), MaxCount, W, lambda);
    ...
    save(sprintf('data/pnlssmodel_b%d_A%d_up%d_%s_na%d_nx%s.mat',benchmark,Alevels(ia),upsamp,dataname, min_na, sprintf('%d',nx)), 'model', 'err')
    ...
    print(sprintf('FIGURES/TDOMRES_PNLSS_b%d_A%d_up%d_%s_na%d_nx%s.eps',benchmark,Alevels(ia),upsamp,dataname,min_na,sprintf('%d',nx)), '-depsc')    
    disp(' ')
