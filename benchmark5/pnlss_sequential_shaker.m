...
PerOrNot = [true true true true true true true];  % true if periodic; false if not
...
load(sprintf('data/b%d_shaker_A%d_up%d_%s',benchmark,Alevels(1),upsamp,dataname))
u = sf;
...
for ia=1:length(Alevels)
    load(sprintf('data/b%d_shaker_A%d_up%d_%s',benchmark,Alevels(ia),upsamp,dataname), 'sf', 'y')
    u = sf;
...
%         uc = u(:);
%         yc = y(:);
        u = reshape(u, [Nt*P, 1, R]);  u = u(1:Nt,1,R);
        y = reshape(y, [Nt*P, 1, R]);  y = y(1:Nt,1,R);
        utest = reshape(utest, [Nt, 1, 1]);
        ytest = reshape(ytest, [Nt, 1, 1]);
        uval = reshape(utest, [Nt, 1, 1]);
        yval = reshape(yval, [Nt, 1, 1]);
...
%  	[~, y_mod, models_pnlss] = fLMnlssWeighted(uc, yc, models_pnlss(end-5), MaxCount, W, lambda);
    ...
    save(sprintf('data/pnlssmodel_b%d_shaker_A%d_up%d_%s_na%d_nx%s.mat',benchmark,Alevels(ia),upsamp,dataname, min_na, sprintf('%d',nx)), 'model', 'err')
    ...
    if PerOrNot(ia)
        ...
        plot(t(1:Nt), plottime)
    end
    xlabel('Time (s)')
    ylabel('Output (errors)')
    legend('output','linear error','PNLSS error')
    title('Estimation results')
    print(sprintf('FIGURES/TDOMRES_PNLSS_shaker_b%d_A%d_up%d_%s_na%d_nx%s.eps',benchmark,Alevels(ia),upsamp,dataname,min_na,sprintf('%d',nx)), '-depsc')    
    disp(' ')
...
save('./data/seqpnlssmodels_shaker.mat', 'Alevels', 'errormeasures', 'seqmodels');