...
%     TYPICAL_x = oscillator.Fex1/(2*D*M*om^2);
    ...
    [X{iex},~] = solve_and_continue(y0, ...
        ...
        Om_s,Om_e,ds,Sopt);
	Sols{iex} = [X{iex}(end,:)' sqrt([1 0.5*ones(1,2*H)]*(kron(eye(2*H+1),PHI_L2)*X{iex}(1:end-1,:)).^2)'...
        atan2d(-PHI_L2*X{iex}(2*n+(1:n),:),PHI_L2*X{iex}(n+(1:n),:))'];
end
...
%     legend(aa(iex), sprintf('F = %.2f', exc_lev(iex)));
    ...
    plot(Sols{iex}(:,1)/2/pi, Sols{iex}(:,3), '-', 'Color', colos(iex,:)); hold on
    plot(Solspnlss{iex}(:,1)/2/pi, Solspnlss{iex}(:,3), '.--', 'Color', colos(iex,:)); hold on
%     legend(aa(iex), sprintf('F = %.2f', exc_lev(iex)));
end
...
print(sprintf('./fig/pnlssfrf_A%.2f_Amp_nx%s_mds%d.eps',Alevels(ia),sprintf('%d',nx),mds), '-depsc')
...
print(sprintf('./fig/pnlssfrf_A%.2f_Phase_nx%s_mds%d.eps',Alevels(ia),sprintf('%d',nx),mds), '-depsc')
end