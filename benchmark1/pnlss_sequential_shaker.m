...
load(sprintf('./data/ode45_multisine_shaker_A%.2f_F%d.mat',Alevels(1), fs))
u = sf;
...
for ia=1:length(Alevels)
    load(sprintf('./data/ode45_multisine_shaker_A%.2f_F%d.mat',Alevels(ia), fs), 'u', 'y', 'sf');
    u = sf;
...
%  	[~, y_mod, models_pnlss] = fLMnlssWeighted(uc, yc, models_pnlss(end-5), MaxCount, W, lambda);
    ...
    save(sprintf('./data/pnlssmodel_shaker_A%.2f_F%d_nx%s.mat', Alevels(ia), fs, sprintf('%d',nx)), 'model', 'err')
    ...
    print(sprintf('./fig/TDOMESTRESS_PNLSS_shaker_A%.2f_F%d_nx%s.eps', Alevels(ia), fs, sprintf('%d',nx)), '-depsc')
    disp(' ')
