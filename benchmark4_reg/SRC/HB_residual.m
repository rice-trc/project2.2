...
switch lower(analysis_type)
    ...
    case {'nma','nonlinear modal analysis'}
        ...
        % Modal mass
        a = exp(log(10)*X(end));
        da = log(10)*exp(log(10)*X(end))*fliplr(eye(1,size(X,1)));
        ...
        % Modal frequency
        Om = X(end-2);
        dOm = zeros(1,length(X)); dOm(end-2) = 1;
        
        % Modal damping ratio
        del = X(end-1);
        ddel = zeros(1,length(X)); ddel(end-1) = 1;
...
for nl=1:length(nonlinear_elements)
    ...
    % If the nonlinear element is hysteretic (e.g. elastic Coulomb
    ...
    if nonlinear_elements{nl}.islocal
        ...
        switch lower(nonlinear_elements{nl}.type)
            ...
            case 'elasticdryfriction'
                % Set stiffness of elastic Coulomb element
                k = nonlinear_elements{nl}.stiffness;
                fc = nonlinear_elements{nl}.friction_limit_force;
                
                % Initialize force vector and slider position
                fnl = zeros(size( qnl ));
                qsl = zeros(size( qnl ));
