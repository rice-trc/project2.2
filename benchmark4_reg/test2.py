...
def identify(data, nlx, nly, nmax=25, info=2, fnsi=False):
    ...
    if fnsi:
        # FNSI can only use 1 realization
        R = 1
        T1 = np.r_[data.npp*data.Ntr, np.r_[0:(R-1)*data.npp+1:data.npp]]
        sig = deepcopy(data.sig)
        # This is stupid, but unfortunately nessecary
        sig.y = sig.y[:, :, 0][:, :, None]
        sig.u = sig.u[:, :, 0][:, :, None]
...
def plot(res, data, p):
    ...
    plt.figure()
    plt.plot(db(res.errvec))
    imin = np.argmin(res.errvec)
    plt.scatter(imin, db(res.errvec[imin]))
...
def ident(data, nlx, nly, n, r, subscan=True):
    errvec = {}
    models = {}
...
data3 = partion_data(raw_data3)
res31 = identify(data3, nlx, nly, nmax=nmax, info=info, fnsi=True)

subscan = False
models, res3 = ident(data3, nlx, nly, n=2, r=5, subscan=subscan)
...
figs1 = plot(res31, data3, p=1)
figs2 = plot_bla(res31, data3, p=1)
